...
augroup endwise " {{{1
  ...
  autocmd FileType vim
        \ let b:endwise_addition = '\=submatch(0)=="augroup" ? submatch(0) . " END" : submatch(0)[0:1]=="fu" ? "endf" : submatch(0)=="if" ? "end" : "end".submatch(0)' |
        \ let b:endwise_words = 'fu\%[nction],wh\%[ile],if,for,try,au,augroup' |
